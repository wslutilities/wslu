...
function gen_startup {
	...
	if [[ "$commd" != "" ]]; then
		...
		if [[ $isService -eq 1 ]]; then
			commd="wsl.exe -d $WSL_DISTRO_NAME -u root service $commd start"
			name="$commd"
		else
...
Import-Module 'C:\\WINDOWS\\system32\\WindowsPowerShell\\v1.0\\Modules\\Microsoft.PowerShell.Utility\\Microsoft.PowerShell.Utility.psd1';\$action = New-ScheduledTaskAction -Execute 'C:\\Windows\\System32\\wscript.exe'  -Argument '$script_location_win\\runHidden.vbs $nname';
