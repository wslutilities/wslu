...
function gen_startup {
	...
	if [[ "$commd" != "" ]]; then
		...
		# service
			...
			commd="wsl.exe -d $WSL_DISTRO_NAME -u $wa_gs_user service $commd start"
		else
		# command
			# handling no name given case
			if [[ "$name" = "" ]]; then
				name=$(basename "$(echo "$commd" | awk '{print $1}')")
			fi
			commd="wsl.exe -d $WSL_DISTRO_NAME -u $wa_gs_user $commd"
		fi

		# shellcheck disable=SC2028
		tee "$(wslpath "$(wslvar -s TMP)")"/tmp.ps1 << EOF
Import-Module 'C:\\WINDOWS\\system32\\WindowsPowerShell\\v1.0\\Modules\\Microsoft.PowerShell.Utility\\Microsoft.PowerShell.Utility.psd1';\$action = New-ScheduledTaskAction -Execute 'C:\\Windows\\System32\\wscript.exe'  -Argument '$script_location_win\\runHidden.vbs $commd';
\$trigger =  New-ScheduledTaskTrigger -AtLogOn -User \$env:userdomain\\\$env:username; \$task = New-ScheduledTask -Action \$action -Trigger \$trigger -Description \"Start service $name from $WSL_DISTRO_NAME when computer start up; Generated By WSL Utilities\";
Register-ScheduledTask -InputObject \$task -TaskPath '\\' -TaskName 'WSLUtilities_Actions_Startup_$name';
EOF
		...
		if winps_exec "$script_location_win"\\sudo.ps1 "$tpath"\\tmp.ps1; then
			...
			echo "${info} Startup \"${name}\" added."

		else
			rm -rf "$tpath_linux/tmp.ps1"
			echo "${error} Adding Startup \"${name}\" failed."
			exit 1
		fi
	else
